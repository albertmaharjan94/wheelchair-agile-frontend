...
function ShowMap() {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:3001/api/tracker/" + selectedUser)
      .then((response) => {
  ...
  useEffect(() => {
    ...
    if (selectedDate !== undefined) {
      ...
      axios
        .get(`http://localhost:3001/api/tracker${url}`)
        .then((response) => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3001/api/tracker")
      .then((response) => {
  ...
  useEffect(() => {
    if (map !== undefined) map.setView(center);
  }, [center]);
  ...
  return (
    <>
      <div
        id="showMap"
        className="w-100 mapSearch d-flex justify-content-between"
      >
        ...
        {trackers.map((tracker) => {
          return (
            <Marker
              ...
              position={tracker.location.coordinates}
              eventHandlers={{
                click: () => {
          ...
          <CircleMarker center={center} radius={5}>
            <Tooltip direction="top" offset={[0, -50]} opacity={1} permanent>
              <Avatar
                size="40"
                round={true}
                name={activeTracker.user.fullname}
              ></Avatar>{" "}
