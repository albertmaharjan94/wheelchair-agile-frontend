...
const Dashboard = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3001/api/auth/all")
      .then((response) => {
  ...
  const getData = async () => {
    // alert(userId);
    await axios
      .get("http://localhost:3001/api/activity/user/" + userId)
      .then((response) => {
    ...
    try {
      if (selectedUser !== undefined) {
        ...
        if (selectedUser.activity !== undefined) {
          ...
          if (
            selectedUser.activity.data !== undefined &&
            selectedUser.activity.category !== undefined
          ) {
  ...
  const [series, setSeries] = useState(seriesFun);

  const [option, setOption] = useState(optionFun);
